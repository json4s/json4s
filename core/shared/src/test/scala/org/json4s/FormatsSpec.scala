...
class FormatsSpec extends AnyWordSpec {
  ...
  "issue#550 avoid scalac bug" in {
    val a = new TypeHints {
      override val hints: List[Class[?]] = List.empty
      override def hintFor(clazz: Class[?]): Option[String] = None
      override def classFor(hint: String, parent: Class[?]): Option[Class[?]] = None
    }
    a + a
  }
}
