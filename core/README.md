...
Add dependency to your project description:
    ...
    val lift_json = "org.scalatra.json4s" %% "json4s" % "XXX"
...
Module lift-json-ext contains extensions to extraction and serialization. Following types are supported.
    ...
    // Lift's box
    implicit val formats = org.json4s.DefaultFormats + new JsonBoxSerializer

    // Scala enums
    implicit val formats = org.json4s.DefaultFormats + new EnumSerializer(MyEnum)
    // or
    implicit val formats = org.json4s.DefaultFormats + new EnumNameSerializer(MyEnum)

    // Joda Time
    implicit val formats = org.json4s.DefaultFormats ++ JodaTimeSerializers.all
