...
import com.tt.json4s.native
import com.tt.json4s.native.Serialization
...
class FullTypeHintExamples extends TypeHintExamples {
  ...
  "Default recursive with type hints example" in {
    ...
    val ser = swrite(pw)
    assert(ser == """{"name":"zoltan","bird":{"jsonClass":"com.tt.json4s.Chicken","eggs":3}}""")
    assert(read[PlayerWithBird]("""{"name":"zoltan"}""") == pw)
  }

  "Deserialization succeeds when a field name matching typeHintFieldName exists" in {
    val ser = """{"jsonClass":"com.tt.json4s.Chicken"}"""
    assert(read[NotTypeHint](ser) == NotTypeHint("com.tt.json4s.Chicken"))
  }
}
